...
- [code] avg_empty: {'例外': 8, '0を返す': 7} (安定性 0.53)
- [code] fib_index: {'fib(0)=0,fib(1)=1,fib(2)=1': 8, 'ERR': 7} (安定性 0.53)
- [code] clamp_pct: {'0-100%表記': 11, '他(25.00%)': 3, '他(25.0%)': 1} (安定性 0.73)
- [code] dup_count: {'他(5)': 8, '他(3)': 4, '他({1: 2, 3: 3})': 1, '重複した値の種類数(2)': 2} (安定性 0.53)
- [inst_ja] meta_length_limit: {'軽微超過(〜99字)': 7, '制限遵守(〜55字)': 8} (安定性 0.53)
- [io] txt_clarify: {'逆質問(不足を指摘)': 7, '手順/コマンドを答える': 8} (安定性 0.53)
- [io_en] io_date_fmt: {'as written (March 5, 2026)': 8, 'ISO(zero-padded)': 7} (安定性 0.53)
- [io_en] io_range_count: {'midpoint (4)': 7, 'range kept': 8} (安定性 0.53)
- [io_en] txt_sum_length: {'short paragraph (<400 chars)': 11, 'long (>=400 chars)': 4} (安定性 0.73)
- [io_en] txt_clarify: {'asks back (points out missing input)': 8, 'gives instructions/commands': 7} (安定性 0.53)
...
---
生成元プロファイル: code(N=5, ?) / code_en(N=5, http://localhost:8000) / inst_en(N=5, http://localhost:8000) / inst_ja(N=5, http://localhost:8000) / io(N=5, http://localhost:8000) / io_en(N=5, http://localhost:8000) / sql_en(N=5, http://localhost:8000) / sql_ja(N=5, http://localhost:8000)
