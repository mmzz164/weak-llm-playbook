...
`--bypass` は許可リストの代わりに `--dangerously-skip-permissions` を子に渡す。手軽だが、子は**全ツール**(Bash・ファイル書き込み・全MCP)を使えるようになり、しかも外部コンテンツ(注入された命令を含み得る)を処理する。信頼できない素材を読むタスクでは `--allowed` を推奨。
