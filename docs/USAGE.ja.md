...
これにより、fixループを弱い操作者——弱LLM自身を含む——に
...
3=対象外。対話エージェントは指示を「説得」で破れるが、手順が自分の実行物でなければ飛ばしようがない——それがこの形の狙い。
...
改訂版タスクを書き出し(スカラー・件数は多数派で自動ピン。多数派のないタイは勝手に
固定せず「★要記入」行になる。顔ぶれの割れはIDを固定しても使い回せないので
「★要記入」行=並び順・範囲・フィルタを書く欄になる)、それをK回再測定して穴の
before → after を報告する。★要記入が残った場合は「何を列挙するか指示に1行書き足して
再実行」という次の一手を、コピーできる再実行コマンド付きで表示する。

エージェントの起動コマンドは `--cmd` > `$WEAK_LLM_AGENT_CMD` > `claude` の順で解決。子は呼び出し元セッションの環境を継承するので、claude-local等のラッパーセッション内から呼べば素の `claude` がそのまま同じ構成で動く——**既定=「今の自分自身で実行」**。素のターミナルから使うならシェルプロファイルに `export WEAK_LLM_AGENT_CMD=...` を1行。どのコマンドで走るかは実行時に必ず表示される。子は**既定で** `--dangerously-skip-permissions` 付きで走る。`--allowed` を渡すと許可リスト方式に切り替わる(`--no-bypass` で素の権限確認)。トレードオフ: バイパスされた子は全ツール(Bash・ファイル書き込み・全MCP)を持ったまま、注入された命令を含み得る外部コンテンツを処理する——信頼できない素材には `--allowed` を。
